...
class Dariustoolkit(commands.Cog):
    ...
    async def vc(self, ctx):
        ...
        em.clear_fields()
        em.add_field(name=f"Initialting protocol: {randprotocol}", value=f"Loading . . . done!", inline=False)
        em.add_field(name=f"Connection to VC:", value=f"[▓▓░░░░░░░░ 18%]", inline=False)
        await thingy.edit(embed=em)
        wait = await self._randshitslow(5)
        asyncio.sleep(wait)
        em.clear_fields()
        em.add_field(name=f"Initialting protocol: {randprotocol}", value=f"Loading . . . done!", inline=False)
        em.add_field(name=f"Connection to VC:", value=f"[▓▓▓▓▓░░░░░ 43%]", inline=False)
        await thingy.edit(embed=em)
        wait = await self._randshitslow(4)
        asyncio.sleep(wait)
        em.clear_fields()
        em.add_field(name=f"Initialting protocol: {randprotocol}", value=f"Loading . . . done!", inline=False)
        em.add_field(name=f"Connection to VC:", value=f"[▓▓▓▓▓▓▓░░░ 74%]", inline=False)
        await thingy.edit(embed=em)
        wait = await self._randshitslow(3)
        asyncio.sleep(wait)
        em.clear_fields()
        em.add_field(name=f"Initialting protocol: {randprotocol}", value=f"Loading . . . done!", inline=False)
        em.add_field(name=f"Connection to VC:", value=f"[▓▓▓▓▓▓▓▓▓░ 95%]", inline=False)
        await thingy.edit(embed=em)
        wait = await self._randshitslow(2)
        asyncio.sleep(wait)
        em.clear_fields()
